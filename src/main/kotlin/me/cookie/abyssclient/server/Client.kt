...
val playerCache: Cache<String, Player> = Caffeine.newBuilder()
    .maximumSize(1_000)
    .expireAfterWrite(
        60,
        TimeUnit.MINUTES
                ...
                while (running.get()) {
                    ...
                    if (compReceivedBytes != null) {
                        ...
                        if (list[0] == "player") {
                            ...
                            val state = list[2]
                            queue.remove(uuid)
                            if (state.toBooleanStrict())
                                playerCache.put(uuid, Player(uuid))
                        }
                    }
                    // Check for stuff in queue
                    if (queue.isNotEmpty()) sendCompressed("player\r${queue.first}\r")
                }
                client.close()
            }
        }
 ...
 */
suspend fun WebSocketSession.readCompressed(buf: ByteArray): String {
