...
object EventManager {
    ...
    private fun isMethodBad(method: Method): Boolean {
        return method.parameterTypes.size != 1 || !method.isAnnotationPresent(EventAnnotation::class.java)
    }
    ...
    operator fun get(clazz: Class<out Event?>): ArrayList<EventData>? {
        return REGISTRY_MAP[clazz]
    }

    fun cleanMap(removeOnlyEmptyValues: Boolean) {
        val iterator: MutableIterator<Map.Entry<Class<out Event?>, ArrayList<EventData>?>> = REGISTRY_MAP.entries.iterator()
    ...
    fun register(method: Method, o: Any?) {
        ...
        val methodData = EventData(o, method, method.getAnnotation(EventAnnotation::class.java).value)
